...
\textcolor{blue}{COMENTARIO: No es necesario encerrar en un recuadro la prueba de los lemas. Si te estorban los recuadros, quítalos.}
\begin{lemma}
...
\begin{lemma}
    ...
    \textcolor{blue}{Poner aquí las condiciones de ROuth Hurwitz}\\
    
    \textcolor{blue}{donde $k_{P2}':=\frac{k_{P2}}{L}$, $k_{I2}':=\frac{k_{I2}}{L}$  $\bar{\gamma}_1:=\frac{f_r}{J}-\frac{\gamma}{J}>0 $ y $\bar{\gamma}_2:=\frac{3}{2J}p\phi>0$.}

    %\begin{proof}
        
    %\end{proof}
\end{lemma}
\textcolor{blue}{Demostración:}\textcolor{blue}{ Bajo las definiciones de $k_{I2}'$, $k_{P2}'$, $\bar{\gamma}_1$ y $\bar{\gamma}_2$ que se muestran en el enunciado del lemma, la matriz $A_2$ de $\Sigma_2^{\mathrm{lin}}$ es
		$$\bar A_2:=\begin{bmatrix}
			0 & -k_{I3} & -k_{P3} & 1 \\
			0 & 0 & 1 & 0 \\
			0 & 0 & -\bar \gamma_1  & -\bar\gamma_2 \\
			-k_{I2}' & k_{P2}'k_{I3} & k_{P2}'k_{P3}& -(\frac{R}{L}+k_{P2}')
		\end{bmatrix}$$
	}
	%	donde $\bar\gamma_1:=\frac{f_r-\gamma}{J}>0,$ $\bar \gamma_2:=\frac{3}{2J}p\phi>0$, $k_{I2}'=\frac{k_{I2}}{L}$, $k_{P2}'=\frac{k_{P2}}{L}$.}
	
	\textcolor{blue}{ COMENTARIO: poner aquí enseguida el polinomio característico. Luego decir que se utilizó matlab para obtener loas condiciones. El desarrollo que tienes abajo te lleva a las condiciones que obtuviste en MATLAB? Si es así, olvida lo de mencionar matlab y deja el desarrollo.}
...
\end{align}
\textcolor{blue}{Poner $\square$ o $\blacksquare$ cuando finalice una prueba. Escoge pf alguno de los dos y siempre usarlo }
...
\textbf{Paso 3.} Encontrar $k_{P2}'>0$ suficientemente grande tal que satisfaga $b_1$ y $c_1$\\\textcolor{blue}{Ponerlo en términos de condiciones que dependan de las ganancias}\\\hline
\end{tabular}
\vspace{1em}
...
Note de la ecuación anterior que a medida que $k_{P2}$ toma valores más grandes, la parte positiva domina por lo que \eqref{eq: cond_b1} se satisface. Entre más grande es $k_{p2}$, más grande es $\varrho$. Esto implica que la cuarta condición \eqref{eq: cond_c1}:
\begin{align}
...
\textcolor{blue}{COMENTARIO: Una vez que encuentres las ganancias verificar que la $\bar{A}_2$, con esas ganancias, es Hurwitz.}
