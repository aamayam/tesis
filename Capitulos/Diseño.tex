...
y se logra con un valor de la relación de velocidad punta de $\lambda^\star \approx 7$. 
\\
...
obteniéndose $\lambda^* = 7.96$ y $C_p^{\text{max}} = 0.411$. La figura \ref{fig:cp_lambda} 
muestra la curva característica $C_p$ vs $\lambda$ para estos parámetros.
...
El punto óptimo $\lambda^* = 7.96$ corresponde al máximo coeficiente de potencia 
$C_p^{\text{max}} = 0.411$.}
\label{fig:cp_lambda}
\end{figure}
...
\begin{lemma}
	...
	\bar{z} :=
		\begin{bmatrix}
			-\frac{2R}{3p\phi k_{\textrm{I2}}}(T^*_m - f_r\omega^*_m) \\[0.3cm]
			\frac{2}{3p\phi k_{\textrm{I3}}}(T^*_m - f_r\omega^*_m) \\[0.3cm]
			\omega^*_m \\[0.3cm]
			\frac{2}{3p\phi}(T^*_m - f_r\omega^*_m)
	\end{bmatrix}
	\end{align}
    donde $T^*_m := r\frac{\kappa_1 v_w C_{p,0}(\lambda^*)}{\lambda^*}$ es el torque mecánico en el 
    equilibrio, $\lambda^* := \frac{r\omega_m^*}{v_w}$ y $\omega_m^*$ es la referencia de velocidad 
    angular.
    ...
    \begin{proof}
        ...
        \begin{align}
        z_4 - [k_{\textrm{P3}}(z_3 - \omega^*_m) + k_{\textrm{I3}}z_2] &= 0 \label{eq:ss1} \\
        ...
        -\frac{R+k_{\textrm{P2}}}{L}z_4 + \frac{k_{\textrm{P2}}k_{\textrm{P3}}}{L}(z_3 - \omega^*_m) + \frac{k_{\textrm{P2}}k_{\textrm{I3}}}{L}z_2 - \frac{k_{\textrm{I2}}}{L}z_1 &= 0 \label{eq:ss4}
        \end{align}
        ...
        Por otro lado, de la ecuación \eqref{eq:ss3}, usando \eqref{eq:eq1} y $\lambda^* = \frac{r\omega^*_m}{v_w}$, se obtiene:
        \begin{equation}
        \bar{z}_4 = \frac{2}{3p\phi}(T^*_m - f_r\omega^*_m) \label{eq:eq3}
        \end{equation}

        Combinando \eqref{eq:eq2} y \eqref{eq:eq3}:
        \begin{equation}
        \bar{z}_2 = \frac{1}{k_{\textrm{I3}}}\bar{z}_4 = \frac{2}{3p\phi k_{\textrm{I3}}}(T^*_m - f_r\omega^*_m) \label{eq:eq4}
        \end{equation}

        Finalmente, sustituyendo $\bar{z}_3 = \omega^*_m$ y \eqref{eq:eq3} en \eqref{eq:ss4}:
        \begin{equation}
        \bar{z}_1 = -\frac{R}{k_{\textrm{I2}}}\bar{z}_4 = -\frac{2R}{3p\phi k_{\textrm{I2}}}(T^*_m - f_r\omega^*_m) \label{eq:eq5}
        \end{equation}
...
donde $\gamma$ está definida como la siguiente derivada evaluada en el punto $\lambda=\lambda_\star$
\begin{align}\nonumber
	\gamma:= \frac{r\kappa_1}{Jv_w}\left.\frac{\partial}{\partial \lambda}\left(\frac{C_{p,0}(\lambda)}{\lambda}\right)\right|_{\lambda=\lambda_\star}.
\end{align}
Desarrollando la derivada, la constante $\gamma$ se reduce a la expresión
\begin{align}
	\gamma=&\left.\frac{r\kappa_1}{Jv_w} \left[\frac{1}{\lambda}\frac{\partial }{\partial \lambda}C_{p,0}(\lambda)-\frac{1}{\lambda^2}C_{p,0}(\lambda)\right]\right|_{\lambda=\lambda_\star}\nonumber\\
	=& - \frac{1}{\lambda_\star^2}\frac{r\kappa_1}{Jv_w} C_{p,0}(\lambda_\star) \label{eq:gamma}
	\end{align}
%\begin{align}
...
\begin{lemma}
    ...
    \eqref{barz}, es exponencialmente estable si y solo si se cumplen las siguientes condiciones:
    \begin{align}
...
\subsection*{Algoritmo para encontrar las ganancias}

El siguiente algoritmo proporciona un procedimiento sistemático para encontrar un conjunto de ganancias que satisfagan las condiciones \eqref{eq:cond1}--\eqref{eq:cond4}.
...
ya que el término $\bar{k}_{\textrm{I3}}k_{\textrm{P2}}'$ crece linealmente mientras que el término del denominador decrece. Por lo tanto, existe un valor de $k_{\textrm{P2}}'$ suficientemente grande que garantiza que las condiciones de Routh-Hurwitz se satisfagan.

\begin{lemma}
    Las condiciones \eqref{eq:cond1}--\eqref{eq:cond4} admiten solución. \tripletriang
    
    \begin{proof}
        Para demostrar la existencia de solución, se aplicó el algoritmo presentado anteriormente 
        con los parámetros del sistema dados en \textcolor{blue}{[referencia a tabla/sección de 
        parámetros]}. Los valores numéricos relevantes son $\frac{R}{L} = 103.5493$, 
        $\bar{\gamma}_1 = 0.000117$ y $\bar{\gamma}_2 = 1.5328$.
        
        Siguiendo el Paso 1 del algoritmo, se fijaron las ganancias del lazo externo como:
        \begin{equation}
            k_{\textrm{P3}} = 10, \quad k_{\textrm{I3}} = 10
        \end{equation}
        
        Conforme al Paso 2, se estableció la relación $k_{\textrm{I2}}' = \frac{1}{k_{\textrm{P2}}'}$.
        
        Para el Paso 3, se realizó una búsqueda incremental de $k_{\textrm{P2}}'$ hasta encontrar 
        un valor que satisficiera las condiciones \eqref{eq:cond3} y \eqref{eq:cond4}. El valor 
        $k_{\textrm{P2}}' = 10$ resultó en el siguiente conjunto de ganancias normalizadas:
        \begin{equation}
            k_{\textrm{P2}}' = 10, \quad k_{\textrm{I2}}' = 0.1
        \end{equation}
        
        que corresponde a las ganancias originales:
        \begin{equation}\label{eq:gains_solution}
            k_{\textrm{P2}} = 0.0355, \quad k_{\textrm{I2}} = 0.000355, \quad 
            k_{\textrm{P3}} = 10, \quad k_{\textrm{I3}} = 10
        \end{equation}
        
        La verificación numérica confirmó que todas las condiciones de Routh-Hurwitz se satisfacen:
        \begin{align*}
            a_3 &= 113.67 > 0 \quad \checkmark \\
            a_0 &= 1.53 > 0 \quad \checkmark \\
            b_1 &= 166.34 > 0 \quad \checkmark \\
            c_1 &= 153.77 > 0 \quad \checkmark
        \end{align*}
        
        Adicionalmente, se verificó que la matriz $\bar{A}_2$ con estas ganancias es Hurwitz. 
        Los valores propios de $\bar{A}_2$ son:
        \begin{equation} \label{eq:eigenvalues}
            \lambda_1 = -112.193, \quad \lambda_2 = -0.736 + 0.907i, \quad \lambda_3 = -0.736 - 0.907i, \quad \lambda_4 = -0.010008
        \end{equation}
        
        Como todos los valores propios tienen parte real negativa, se confirma que $\bar{A}_2$ es 
        Hurwitz y, por lo tanto, el punto de equilibrio $\bar{z}$ es exponencialmente estable.
    \end{proof}
\end{lemma}

\subsection{Análisis del lugar geométrico de las raíces}

Con el objetivo de complementar el análisis de estabilidad presentado en la
sección anterior y visualizar el efecto de la ganancia $k'_{P2}$ sobre la
ubicación de los valores propios del sistema linealizado $\Sigma_2^{\text{lin}}$,
se construyó el lugar geométrico de las raíces (LGR) de la matriz $\bar{A}_2$
en función del parámetro $k'_{P2}$, manteniendo fijas las ganancias del lazo
externo $k_{P3} = 10$, $k_{I3} = 10$ y la relación $k'_{I2} = 1/k'_{P2}$
establecida en el Paso~2 del algoritmo de sintonización.
\\ 

La figura \ref{fig:root_locus} muestra el LGR completo junto con un detalle
ampliado de los valores propios $\lambda_3$ y $\lambda_4$, que son los de
mayor relevancia dinámica para el análisis de estabilidad de $\Sigma_2$.
Los valores propios $\lambda_1$ y $\lambda_2$, asociados al lazo de corriente
del eje directo $\Sigma_1$, permanecen esencialmente invariantes ante
variaciones de $k'_{P2}$ y se ubican consistentemente en el semiplano
izquierdo lejano al eje imaginario, lo cual es consistente con la estabilidad
incondicional de $\Sigma_1$ demostrada en el Lema 3.2.4.
\\ 

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{root_locus.pdf}
    \caption{Lugar geométrico de las raíces de $\bar{A}_2$ en función de
    $k'_{P2} \in [0.1,\, 10^4]$. Panel izquierdo: LGR completo mostrando
    los cuatro valores propios. Panel derecho: detalle de $\lambda_3$ y
    $\lambda_4$, donde se aprecia la migración desde el semiplano derecho
    hacia el semiplano izquierdo conforme $k'_{P2}$ aumenta.}
    \label{fig:root_locus}
\end{figure} 

El comportamiento observado en el panel derecho de la
figura \ref{fig:root_locus} es coherente con las condiciones de
Routh-Hurwitz establecidas en el Lema 3.2.5. Para valores pequeños de
$k'_{P2}$, las condiciones \eqref{eq:cond3} y \eqref{eq:cond4} no se
satisfacen, lo que se manifiesta en la presencia de valores propios con
parte real positiva. A medida que $k'_{P2}$ crece, los polos $\lambda_3$ y
$\lambda_4$ migran hacia el semiplano izquierdo, cruzando el eje imaginario
para un valor umbral de $k'_{P2}$ que puede identificarse visualmente en el
zoom. Este comportamiento valida geométricamente la existencia del umbral
mínimo de ganancia establecido analíticamente en el Lema 3.2.6.
\\ 

Para el valor seleccionado $k'_{P2} = 10$ — marcado con un cuadro en la
figura — los cuatro valores propios presentan parte real estrictamente
negativa, confirmando que $\bar{A}_2$ es Hurwitz y que el punto de
equilibrio $\bar{z}$ definido en \eqref{eq:sigma_2} es exponencialmente
estable, en concordancia con los valores numéricos reportados en \eqref{eq:eigenvalues}.